...
class OpenrouterDualLlmClient implements DualLlmClient {
  private client: OpenAI;
  private model: string;

  constructor(apiKey: string | undefined, model: string) {
    logger.debug({ model }, "[dualLlmClient] OpenRouter: initializing client");
    // OpenRouter typically doesn't require API keys, use dummy if not provided
    this.client = new OpenAI({
      apiKey: apiKey || "EMPTY",
  ...
  async chatWithSchema<T>(
    ...
    // OpenRouter supports JSON schema via guided decoding
    // Try OpenAI-compatible structured output first
    try {
      const response = await this.client.chat.completions.create({
        model: this.model,
      ...
      logger.debug(
        { model: this.model },
        "[dualLlmClient] OpenRouter: structured output not supported, using prompt fallback",
      );
...
const dualLlmClientFactories: Record<SupportedProvider, DualLlmClientFactory> =
  {
    ...
    openrouter: (apiKey, model) => {
      if (!model) throw new Error("Model name required for OpenRouter dual LLM");
