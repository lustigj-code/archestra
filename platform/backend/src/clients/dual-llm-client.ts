...
class XaiDualLlmClient implements DualLlmClient {
  private client: OpenAI;
  private model: string;

  constructor(apiKey: string | undefined, model: string) {
    logger.debug({ model }, "[dualLlmClient] x.ai: initializing client");
    // x.ai typically doesn't require API keys, use dummy if not provided
    this.client = new OpenAI({
      apiKey: apiKey || "EMPTY",
  ...
  async chatWithSchema<T>(
    ...
    // x.ai supports JSON schema via guided decoding
    // Try OpenAI-compatible structured output first
    try {
      const response = await this.client.chat.completions.create({
        model: this.model,
      ...
      logger.debug(
        { model: this.model },
        "[dualLlmClient] x.ai: structured output not supported, using prompt fallback",
      );
