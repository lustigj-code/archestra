 ...
 * OpenRouter Adapter
 *
 * OpenRouter exposes an OpenAI-compatible API, so this adapter is largely based on the OpenAI adapter.
 * See: https://docs.openrouter.ai/en/latest/features/openai_api.html
 */
  ...
  implements LLMRequestAdapter<OpenrouterRequest, OpenrouterMessages>
{
  ...
  convertToolResultContent(messages: OpenrouterMessages): OpenrouterMessages {
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to OpenRouter/OpenAI format
  ...
  toProviderRequest(): OpenrouterRequest {
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[OpenrouterAdapter] Stripped browser tool results",
        );
      }
    }
    ...
    logger.info(
      ...
      "[OpenrouterAdapter] Building provider request",
    );
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[OpenrouterAdapter] Stripped images from tool result (model does not support images)",
    );
  }
  ...
  implements LLMStreamAdapter<OpenrouterStreamChunk, OpenrouterResponse>
{
  ...
  processChunk(chunk: OpenrouterStreamChunk): ChunkProcessingResult {
    ...
    // Handle usage first - OpenRouter (like OpenAI) sends usage in a final chunk with empty choices[]
...
async function convertToolResultsToToon(
  ...
  // Use OpenAI tokenizer since OpenRouter uses similar tokenization for most models
...
> = {
  ...
  extractApiKey(headers: OpenrouterHeaders): string | undefined {
    // OpenRouter requires API key via Bearer token
  ...
  createClient(
    ...
    // OpenRouter uses OpenAI SDK since it's OpenAI-compatible
    // Use dummy API key if none provided (OpenRouter typically doesn't require auth)
  ...
  extractErrorMessage(error: unknown): string {
    // OpenRouter uses OpenAI SDK error structure
